...
const ImagesDocument = () => {
    ...
    return (
        <>
            ...
            <section className="p-4 bg-white rounded-bl-2xl rounded-br-2xl">
                <div className="md:border border-[#EDEEF4] rounded-xl bg-white">

                    {/* Tabs */}
                    <div className="flex gap-6 p-4 border-b border-[#EDEEF4] flex-wrap">
                        {["Images Library", "Document Library"].map((item) => (
                            <button
                                key={item}
                                onClick={() => setActive(item)}
                                className={`font-medium text-[16px] pb-1 whitespace-nowrap ${
                                    active === item
                                        ? "border-b-2 border-[#2A75FF] text-[#2A75FF]"
                                        : "text-gray-500"
                                }`}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4">
                        ...
                        {/* Upload Button */}
                        <label className="bg-[#2A75FF] text-white px-4 py-2 rounded-lg cursor-pointer text-sm text-center w-full sm:w-auto">
                            {active === "Images Library" ? "Upload Images" : "Upload Files"}
                            <input
                                type="file"
                                multiple
                                    ...
                                    >
                                        {file.type?.startsWith("image") ? (
                                            <img
                                                src={file.preview || URL.createObjectURL(file)}
                                                alt={file.name}
                                                className="h-32 sm:h-24 w-full object-cover rounded"
                                            />
                                        ) : (
                                            <div className="h-32 sm:h-24 flex items-center justify-center bg-gray-100 rounded text-sm">
                                                📄 {file.name}
                                            </div>
                                        )}
                                        <p className="text-xs mt-2 truncate">{file.name}</p>
