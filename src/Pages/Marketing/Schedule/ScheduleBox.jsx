...
const ScheduleBox = () => {
    ...
    return (
        <div className="border border-[#EDEEF4] rounded-[6px]">
            ...
            <div className="flex flex-col md:flex-row gap-5 p-4 md:p-0">
                {/* LEFT SIDE */}
                <div className="w-full md:w-[400px] border md:border-r border-[#EDEEF4] p-4 bg-[#F7F7F8] rounded-md md:rounded-none">
                    <h3 className="text-sm font-medium mb-3 inter">Select Send-On Date</h3>
                    <p className="text-[14px] inter text-[#4C4646] mb-3">
                        Choose the date when this email should be sent.
                    </p>
                    ...
                    <div className="mt-6">
                        ...
                        <div className="flex flex-wrap gap-3 items-end">
                            {["Hours", "Minutes", "Seconds"].map((label) => (
                                <div key={label} className="flex flex-col items-center">
                                    ...
                                    <input
                                        placeholder="00"
                                        className="w-12 md:w-14 h-10 bg-[#9ED0FF] text-white text-center text-lg rounded-md border border-[#5AA9FF]"
                                    />
                                </div>
                            ))}
                            ...
                            <div className="flex flex-col items-center">
                                <span className="text-sm text-gray-700 mb-1">Mode</span>
                                <select className="w-12 md:w-14 h-10 bg-[#9ED0FF] text-white text-center text-sm rounded-md border border-[#5AA9FF]">
                                    <option>AM</option>
                                    <option>PM</option>
                                </select>
                            </div>
                        ...
                        <div className="space-y-2">
                            {["05:00 PM", "05:00 PM", "07:00 PM"].map((time, index) => (
                                <button
                                    key={index}
                                    className={`w-full py-2 rounded-[2px] text-sm ${
                                        index !== 1 ? "bg-[#8FD4FF] text-blue-700" : "border border-[#A6A4A4]"
                                    }`}
                                >
                                    {time}
                                </button>
                ...
                {/* RIGHT SIDE */}
                <div className="flex-1 border border-[#CBCBCB] rounded-md md:rounded-none">
                    <div className="bg-[#006BAE] text-white text-[16px] inter font-medium px-4 py-2 text-center rounded-t-md md:rounded-t-none">
                        View
                    </div>

                    <div className="space-y-3 text-sm p-2 md:p-3">
                        <div className="bg-[#E8F6FF] px-3 py-2 inter font-medium text-[14px] rounded-md">
                            December 1, 2025
                        </div>

                        {["12:00 AM", "01:00 AM", "10:00 AM", "01:00 AM", "01:00 AM"].map((time, index) => (
                            <div
                                key={index}
                                className="border-b border-[#EDEEF4] pb-2 px-3 text-[#4C4646] text-[14px] font-medium inter"
                            >
